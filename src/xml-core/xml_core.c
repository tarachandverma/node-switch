...
#define CONST_SUCCESS			1
	...
	int xmlcore_AddToTail(pool* p, DLinkedList* ll, void* elt){
		llnode* node;
	...
	void* xmlcore_RemoveFromTail(pool* p, DLinkedList* ll){
		void* elt=NULL;
		if(ll->head==ll->tail){
			elt=ll->head->data;
	...
	static void xmlcore_printxrec_list(pool* p, DLinkedList* ll){
		llnode* node;
		x_rec* xrec;
	...
	static void xmlcore_printNodeInfo_list(pool* p, DLinkedList* ll){
		llnode* node;
		node_info *ninfo;
	...
	static DLinkedList* xmlcore_parseXpathNodes(pool* p, char* str){
		node_info *ninfo;
		DLinkedList* ll;
		Tokener* tk;
		char* tok;
		
		ll=xmlcore_getLinkedList(p);		
		tk=toku_getTokenizer(p,str,"/");

		//printf("str:%s\n",str);	
		
		while((tok=toku_next_token(tk))!=NULL){		
			ninfo=apr_palloc(p,sizeof(node_info));
			ninfo->name=tok;
			//printf("tok:%s\n",ninfo->name);	
			xmlcore_AddToTail(p,ll,ninfo);
		}
		return ll;	
	}
	...
	XmlCore* xmlcore_getXmlCore(pool *p){
		...
		c->p=p;
		c->xpath_nodes=xmlcore_getLinkedList(p);
		c->xpath_handlers=xmlcore_getLinkedList(p);
		return c;
	}
	...
	int xmlcore_addXPathHandler(XmlCore* xCore, char* path, int options, const xfunc sfunction, const bfunc bfunction, const efunc efunction, void* udata){
		...
		xrec->xpath_nodes=xmlcore_parseXpathNodes(xCore->p,path);
		xrec->body=NULL;
		

		ret=xmlcore_AddToTail(xCore->p,xCore->xpath_handlers,xrec);
		//xmlcore_printxrec_list(xCore->p,xCore->xpath_handlers);
		return ret;
	}
	
	static int xmlcore_matchNodeDLists(DLinkedList* l1, DLinkedList* l2){
		llnode *node1, *node2;
	...
	static void XMLCALL elementStart(void *data, const char *el, const char **attr){
		...
		//printf("add node:%s\n",el);
		xmlcore_AddToTail(xCore->p,xCore->xpath_nodes,ninfo);
		//xmlcore_printNodeInfo_list(xCore->p,xCore->xpath_nodes);

		//fire start handlers
		node=xCore->xpath_handlers->head;
		while(node!=NULL){
			xrec=(x_rec*)(node->data);
			//xmlcore_printNodeInfo_list(xCore->p,xrec->xpath_nodes);
			if(xmlcore_matchNodeDLists(xrec->xpath_nodes,xCore->xpath_nodes)){
				//printf("node match!\n");
	...
	static void XMLCALL elementEnd(void *data, const char *el){
		...
		//clean up nodes
		ninfo=xmlcore_RemoveFromTail(xCore->p,xCore->xpath_nodes);
		//printf("rem node:%s\n",ninfo->name);
		//xmlcore_printNodeInfo_list(xCore->p,xCore->xpath_nodes);
	}
	...
	static char* xmlcore_parseTextResponse(apr_pool_t* p, int code,char* file){
	...
	char* xmlcore_parseFromStringSourceTextResponse(XmlCore* xCore, char* source){
		int ret;
		ret=xmlcore_parseFromStringSource(xCore,source);
		return xmlcore_parseTextResponse(xCore->p,ret,source);
	}
	char* xmlcore_beginParsingTextResponse(XmlCore* xCore, char* file){
		int ret;
		ret=xmlcore_beginParsing(xCore,file);
		return xmlcore_parseTextResponse(xCore->p,ret,file);
	}
